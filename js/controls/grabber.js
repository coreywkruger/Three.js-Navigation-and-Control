THREE.grabber = function( object, camera, cursor, scalex, scaley, domElement ) {
	...
	function mousedown ( event ) {
		...
		dragStart.copy( _this.getMouseVector( _this.domElement, event.pageX, event.pageY, _this.zOffset ) ).applyEuler(_this.pivot.rotation, _this.pivot.eulerOrder).add(_this.pivotPosition);
	...
	function mousemove ( event ) {
		if (!clicked) return;
		event.stopPropagation();
		dragEnd.copy( _this.getMouseVector( _this.domElement, event.pageX, event.pageY, _this.zOffset ) ).applyEuler(_this.pivot.rotation, _this.pivot.eulerOrder).add(_this.pivotPosition);
		moving = true;
	}
